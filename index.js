...
function createMatchObject(nombreGrupo, jornada, equipoA, equipoB) {
    return {
        ...
        nombreGrupo, jornada, equipoA, equipoB, resultado: null,
        reportedScores: {}, status: 'en_curso', threadId: null
    };
}
...
async function mostrarMensajeEspera() {
    ...
    const channelsToUpdate = [INSCRIPCION_CHANNEL_ID, EQUIPOS_INSCRITOS_CHANNEL_ID, CLASIFICACION_CHANNEL_ID, CALENDARIO_JORNADAS_CHANNEL_ID];
    for (const channelId of channelsToUpdate) {
        try {
            ...
            if (err.code === 10003) { console.warn(`[WARN] El canal de espera con ID ${channelId} no fue encontrado. Se ignora.`); }
            else { console.error(`[ERROR] No se pudo actualizar el canal de espera ${channelId}:`, err); }
        }
    }
}
async function crearHiloDePartido(guild, partido, tipoPartido = 'Grupo') {
    const parentChannel = await client.channels.fetch(MATCH_THREADS_PARENT_ID).catch(() => null);
    if (!parentChannel || parentChannel.type !== ChannelType.GuildText) { return; }
    ...
    let baseChannelName, description;
    ...
    try {
        const thread = await parentChannel.threads.create({ name: threadName, autoArchiveDuration: 1440, type: ChannelType.PrivateThread, reason: `Partido de torneo: ${partido.equipoA.nombre} vs ${partido.equipoB.nombre}`});
        partido.threadId = thread.id;
        await thread.members.add(partido.equipoA.capitanId).catch(()=>{});
        await thread.members.add(partido.equipoB.capitanId).catch(()=>{});
        const arbitroRole = await guild.roles.fetch(ARBITRO_ROLE_ID).catch(() => null);
        if (arbitroRole) { arbitroRole.members.forEach(member => { thread.members.add(member.id).catch(()=>{}); }); }
        ...
        const captainButtons = new ActionRowBuilder().addComponents( new ButtonBuilder().setCustomId(`reportar_resultado_v3_${partido.matchId}`).setLabel("Reportar Resultado / Report Result").setStyle(ButtonStyle.Primary).setEmoji("📊"), new ButtonBuilder().setCustomId(`upload_highlights_${partido.matchId}`).setLabel("Subir Alturas / Upload Highlights").setStyle(ButtonStyle.Secondary).setEmoji("🎬"), new ButtonBuilder().setCustomId(`solicitar_arbitraje_${partido.matchId}`).setLabel("Solicitar Arbitraje / Request Referee").setStyle(ButtonStyle.Danger).setEmoji("⚠️") );
        const adminButtons = new ActionRowBuilder().addComponents( new ButtonBuilder().setCustomId(`admin_modificar_resultado_${partido.matchId}`).setLabel("Modificar Resultado (Admin)").setStyle(ButtonStyle.Secondary).setEmoji("✍️") );
    ...
    } catch (error) { console.error(`[ERROR FATAL] No se pudo crear el hilo del partido.`, error); throw error; }
}
...
client.on('interactionCreate', async interaction => {
    try {
        ...
        try {
            ...
            if (e.code !== 10062) { console.error('Error al enviar mensaje de error de interacción:', e); }
        }
    }
});
async function handleSlashCommand(interaction) {
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) { return interaction.reply({ content: '🇪🇸 No tienes permisos para usar este comando.\n🇬🇧 You do not have permission to use this command.', flags: [MessageFlags.Ephemeral] }); }
...
async function handleButton(interaction) {
    ...
    if (customId.startsWith('panel_')) {
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) { return interaction.reply({ content: 'No tienes permisos para usar los botones del panel.', flags: [MessageFlags.Ephemeral] }); }
        const [panel, type, subtype] = customId.split('_');
        if (type === 'crear') {
            if (torneoActivo) { return interaction.reply({ content: 'Ya hay un torneo activo. Debes finalizarlo antes de poder crear uno nuevo.', flags: [MessageFlags.Ephemeral] }); }
        ...
        if (type === 'add' && subtype === 'test') {
            if (!torneoActivo) { return interaction.reply({ content: 'Primero debes crear un torneo para poder añadir equipos de prueba.', flags: [MessageFlags.Ephemeral] }); }
        ...
        if (type === 'ver' && subtype === 'pendientes') {
            if (!torneoActivo || Object.keys(torneoActivo.equipos_pendientes || {}).length === 0) { return interaction.editReply({ content: 'No hay equipos pendientes de aprobación en este momento.' }); }
        ...
        if (type === 'simular' && subtype === 'partidos') {
            if (!torneoActivo || (torneoActivo.status !== 'fase_de_grupos' && torneoActivo.status !== 'semifinales' && torneoActivo.status !== 'final')) { return interaction.editReply({ content: 'Solo se pueden simular partidos durante una fase activa del torneo.' }); }
            let partidosActivos = findMatch(null, true);
            let partidosASimular = partidosActivos.filter(p => p.threadId && p.status !== 'finalizado');
            if (partidosASimular.length === 0) { return interaction.editReply({ content: 'No hay partidos activos (con hilo creado) para simular en este momento.' }); }
        ...
        if (type === 'finalizar') {
            ...
            saveFullBotData(); 
            await actualizarNombresCanalesConIcono();
            await interaction.followUp({ content: `✅ Torneo "${nombreTorneoFinalizado}" finalizado. La limpieza de canales ha comenzado en segundo plano.`, flags: [MessageFlags.Ephemeral] });
        ...
        await interaction.deferReply({ flags: [MessageFlags.Ephemeral] });
        const [,, guildId] = customId.split('_');
        const roleInfo = Object.values(languageRoles).find(r => r.code === customId.split('_')[1]);
        if (!roleInfo || !guildId) { return interaction.editReply({ content: 'Error: El botón que has pulsado es inválido o antiguo.' }); }
        ...
        try {
            ...
            for (const flag in languageRoles) {
                const role = guild.roles.cache.find(r => r.name === languageRoles[flag].name);
                if (role && member.roles.cache.has(role.id)) { rolesToRemove.push(role); }
            }
            if (rolesToRemove.length > 0) { await member.roles.remove(rolesToRemove, 'Cambiando rol de idioma'); }
            ...
            } else { await interaction.editReply({ content: `Error: El rol para ${roleInfo.name} no existe. Por favor, contacta a un administrador.` }); }
        } catch (error) { console.error('Error al asignar rol por botón:', error); await interaction.editReply({ content: 'Hubo un error al intentar asignarte el rol.' }); }
    }
    else if (customId === 'inscribir_equipo_btn') {
        if (!torneoActivo || torneoActivo.status !== 'inscripcion_abierta') { return interaction.reply({ content: '🇪🇸 Las inscripciones no están abiertas o el torneo ha sido borrado.\n🇬🇧 *Registrations are not open or the tournament was deleted.*', flags: [MessageFlags.Ephemeral] }); }
        ...
        if(!partido) return interaction.reply({content: "Error: No se pudo encontrar el partido para este botón.", flags: [MessageFlags.Ephemeral] });
        ...
        if (customId.startsWith('admin_aprobar_') || customId.startsWith('admin_rechazar_') || customId.startsWith('admin_expulsar_')) {
            const [,, captainId] = customId.split('_');
            const type = customId.split('_')[1];
            if (type === 'expulsar') {
                if (!torneoActivo || torneoActivo.status !== 'inscripcion_abierta') { return interaction.editReply({ content: 'Solo se pueden expulsar equipos durante la fase de inscripción.' }); }
                ...
                if(captainUser) { await captainUser.send(`🇪🇸 Tu equipo **${teamToKick.nombre}** ha sido eliminado del torneo por un administrador.\n🇬🇧 Your team **${teamToKick.nombre}** has been removed from the tournament by an administrator.`).catch(() => {}); }
                ...
                if (type === 'aprobar') {
                    ...
                    const captainUser = await client.users.fetch(captainId).catch(()=>null);
                    if(captainUser) { await captainUser.send(`✅ 🇪🇸 ¡Tu inscripción para el equipo **${equipoPendiente.nombre}** ha sido aprobada!\n\n🇬🇧 Your registration for the team **${equipoPendiente.nombre}** has been approved!`).catch(()=>{}); }
        ...
        if (customId.startsWith('admin_confirm_payment_')) {
            ...
            try { await winner.send({ embeds: [dmEmbed] }); }
            catch (e) { return interaction.editReply({ content: `No se pudo enviar el DM al ganador, pero la acción se ha registrado.` }); }
            const originalMessage = interaction.message;
            ...
            disabledRow.components.forEach(component => { if (component.data.custom_id === customId) { component.setDisabled(true).setLabel('Pago Confirmado'); } });
...
async function handleModalSubmit(interaction) {
    ...
    if (customId.startsWith('crear_torneo_final_')) {
        ...
        if (isPaid) {
            ...
            if (!enlacePaypal || isNaN(prizeCampeon) || isNaN(prizeFinalista)) { return interaction.editReply({ content: 'Debes proporcionar un enlace de PayPal y premios numéricos válidos.' }); }
        }
        ...
        torneoActivo = { 
            nombre, size, isPaid, prizeCampeon, prizeFinalista, status: 'inscripcion_abierta', 
            enlace_paypal: enlacePaypal, equipos_pendientes: {}, equipos_aprobados: {}, 
            canalEquiposId: EQUIPOS_INSCRITOS_CHANNEL_ID, canalGruposId: CLASIFICACION_CHANNEL_ID,
            mensajeInscripcionId: null, listaEquiposMessageId: null, publicGroupsMessageId: null, calendarioMessageId: null,
        ...
        if (teamName.length < 3 || teamName.length > 8) { return interaction.editReply({ content: '🇪🇸 El nombre del equipo debe tener entre 3 y 8 caracteres.\n🇬🇧 *Team name must be between 3 and 8 characters long.*' }); }
        ...
        if (torneoActivo.isPaid) {
            ...
            try { await interaction.user.send({ embeds: [embed], components: [row] }); await interaction.editReply({ content: '✅ 🇪🇸 ¡Revisa tus DMs para las instrucciones de pago!\n🇬🇧 *Check your DMs for payment instructions!*' }); }
            catch { await interaction.editReply({ content: '❌ 🇪🇸 No pude enviarte un DM. Por favor, revisa tu configuración de privacidad.\n🇬🇧 *I could not send you a DM. Please check your privacy settings.*' }); }
        } else {
        ...
        let adminFlag = '🧪'; 
        for (const flag in languageRoles) { const role = interaction.guild.roles.cache.find(r => r.name === languageRoles[flag].name); if (role && adminMember.roles.cache.has(role.id)) { adminFlag = flag; break; } }
        ...
        if (partido.resultado) { return interaction.editReply("Este partido ya tiene un resultado."); }
        ...
        if (otherCaptainResult) {
            if (otherCaptainResult.golesA === golesA && otherCaptainResult.golesB === golesB) {
                ...
                await interaction.channel.send(`**⚠️ ¡Conflicto de resultados!** Los marcadores no coinciden. Vuelvan a reportar.`);
                await interaction.editReply(`Los resultados no coinciden.`);
            }
        } else { await interaction.editReply(`Tu resultado (${golesA}-${golesB}) ha sido guardado. Esperando al otro capitán.`); }
        saveFullBotData();
        ...
        const { partido } = findMatch(matchId);
        if (!partido || !partido.threadId) { return interaction.editReply({ content: '❌ Error: No se pudo encontrar el hilo del partido.' }); }
        const threadChannel = await client.channels.fetch(partido.threadId).catch(() => null);
        if (!threadChannel) { return interaction.editReply({ content: '❌ Error: No se pudo acceder al hilo del partido.' }); }
        const embed = new EmbedBuilder().setColor('#7289DA').setAuthor({ name: `Alturas por ${interaction.user.tag}`, iconURL: interaction.user.displayAvatarURL() }).setTitle('🎬 ¡Nuevas Alturas!').setDescription(`**[Ver Clip](${link})**\n\n${description || ''}`).setTimestamp();
...
async function procesarResultadoFinal(partido, interaction, fromSimulation = false) {
    await updateMatchThreadName(partido);
    if (partido.nombreGrupo) { await actualizarEstadisticasYClasificacion(partido, partido.nombreGrupo, interaction.guild); }
    else { 
        ...
        if (esSemifinal) { await handleSemifinalResult(interaction.guild); }
        else if (esFinal) { await handleFinalResult(); }
    }
    ...
    if (!fromSimulation) {
        ...
        } catch(error) { console.warn("No se pudo enviar mensaje a hilo.", error.message); }
    }
}
function findMatch(matchId, all = false) {
    if (!torneoActivo) return { partido: null };
    const allMatches = [ ...(Object.values(torneoActivo.calendario || {}).flat()), ...(torneoActivo.eliminatorias?.semifinales || []), ...(torneoActivo.eliminatorias?.final ? [torneoActivo.eliminatorias.final] : []) ].filter(Boolean);
...
async function realizarSorteoDeGrupos(guild) {
    ...
    if(torneo.mensajeInscripcionId) {
        try {
            ...
            await msg.edit({ content: 'Las inscripciones han finalizado.', components: [disabledRow] });
    ...
    if (!gruposChannel) { return; }
    await limpiarCanal(CLASIFICACION_CHANNEL_ID);
    torneo.canalGruposId = CLASIFICACION_CHANNEL_ID;
    const embedClasificacion = new EmbedBuilder().setColor('#1abc9c').setTitle(`Clasificación: ${torneo.nombre}`).setDescription('¡Mucha suerte!').setTimestamp();
...
async function verificarYCrearSiguientesHilos(guild) {
    ...
    for (const groupName in torneoActivo.calendario) {
        for (const partido of torneoActivo.calendario[groupName]) {
            ...
            const partidoAnteriorA = torneoActivo.calendario[groupName].find(p => p.jornada === jornadaActual - 1 && (p.equipoA.id === partido.equipoA.id || p.equipoB.id === partido.equipoA.id));
            const partidoAnteriorB = torneoActivo.calendario[groupName].find(p => p.jornada === jornadaActual - 1 && (p.equipoA.id === partido.equipoB.id || p.equipoB.id === partido.equipoB.id));
            if (partidoAnteriorA?.status === 'finalizado' && partidoAnteriorB?.status === 'finalizado') {
...
async function iniciarFaseEliminatoria(guild) {
    ...
    saveFullBotData();
    const embedAnuncio = new EmbedBuilder().setColor('#e67e22').setTitle('🔥 ¡Comienzan las Semifinales! 🔥').addFields({ name: 'Semifinal 1', value: `> ${semifinal1.equipoA.nombre} vs ${semifinal1.equipoB.nombre}` }, { name: 'Semifinal 2', value: `> ${semifinal2.equipoA.nombre} vs ${semifinal2.equipoB.nombre}` });
...
async function handleSemifinalResult(guild) {
    ...
    if (semifinales.every(p => p.status === 'finalizado')) {
        ...
        saveFullBotData();
        const embedAnuncio = new EmbedBuilder().setColor('#f1c40f').setTitle('🏆 ¡La Gran Final! 🏆').setDescription(`**${final.equipoA.nombre} vs ${final.equipoB.nombre}**`);
        const clasifChannel = await client.channels.fetch(torneoActivo.canalGruposId);
        await clasifChannel.send({ embeds: [embedAnuncio] });
    }
}
async function handleFinalResult() {
    ...
    saveFullBotData();
    const embedCampeon = new EmbedBuilder().setColor('#ffd700').setTitle(`🎉 ¡Tenemos un Campeón! 🎉`).setDescription(`**¡Felicidades a ${campeon.nombre} por ganar ${torneoActivo.nombre}!**`).setThumbnail('https://i.imgur.com/C5mJg1s.png').setTimestamp();
    const clasifChannel = await client.channels.fetch(torneoActivo.canalGruposId);
    ...
    if (torneoActivo.isPaid) {
        const adminChannel = await client.channels.fetch(ADMIN_CHANNEL_ID).catch(() => null);
        if(adminChannel) {
            const paymentEmbedCampeon = new EmbedBuilder().setColor('#FFD700').setTitle('🏆 Pagar Premio al CAMPEÓN').addFields({ name: 'Equipo', value: campeon.nombre },{ name: 'Capitán', value: campeon.capitanTag },{ name: 'PayPal', value: `\`${campeon.paypal || 'N/A'}\`` },{ name: 'Monto', value: `${torneoActivo.prizeCampeon}€` });
            const rowCampeon = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`admin_confirm_payment_campeon_${campeon.id}`).setLabel('Confirmar Pago').setStyle(ButtonStyle.Success));
            if (campeon.paypal) { rowCampeon.addComponents(new ButtonBuilder().setLabel('Pagar').setStyle(ButtonStyle.Link).setURL(`https://paypal.me/${campeon.paypal}/${torneoActivo.prizeCampeon}`).setEmoji('💸')); }
            await adminChannel.send({ content: `<@&${ARBITRO_ROLE_ID}>`, embeds: [paymentEmbedCampeon], components: [rowCampeon] });
            const paymentEmbedFinalista = new EmbedBuilder().setColor('#C0C0C0').setTitle('🥈 Pagar Premio al FINALISTA').addFields({ name: 'Equipo', value: finalista.nombre },{ name: 'Capitán', value: finalista.capitanTag },{ name: 'PayPal', value: `\`${finalista.paypal || 'N/A'}\`` },{ name: 'Monto', value: `${torneoActivo.prizeFinalista}€` });
            const rowFinalista = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`admin_confirm_payment_finalista_${finalista.id}`).setLabel('Confirmar Pago').setStyle(ButtonStyle.Success));
            if (finalista.paypal) { rowFinalista.addComponents(new ButtonBuilder().setLabel('Pagar').setStyle(ButtonStyle.Link).setURL(`https://paypal.me/${finalista.paypal}/${torneoActivo.prizeFinalista}`).setEmoji('💸')); }
...
async function actualizarMensajeClasificacion() {
    ...
    for (const groupName in torneoActivo.grupos) {
        ...
        const header = "EQUIPO".padEnd(16) + "PJ  PTS  GF  GC   DG";
        const table = equiposOrdenados.map(e => {
            return `${e.nombre.padEnd(16)}${e.stats.pj.toString().padStart(2)}  ${e.stats.pts.toString().padStart(3)}  ${e.stats.gf.toString().padStart(3)}  ${e.stats.gc.toString().padStart(3)} ${(e.stats.dg >= 0 ? '+' : '') + e.stats.dg.toString().padStart(3)}`;
        }).join('\n');
...
async function actualizarMensajeCalendario() {
    ...
    if (!message) return;
    const newEmbed = EmbedBuilder.from(message.embeds[0]).setDescription('Calendario completo.').setFields([]);
    ...
    for (const groupName of calendarioOrdenado) {
        ...
        for (const jornadaNum in partidosPorJornada) {
            ...
            for (const partido of partidosPorJornada[jornadaNum]) {
                ...
                groupScheduleText += `\`${partido.equipoA.nombre.padEnd(12)}\` ${resultado} \`${partido.equipoB.nombre.padEnd(12)}\`\n`;
            }
            groupScheduleText += '\n';
        }
...
client.on('messageReactionAdd', async (reaction, user) => {
    ...
    if (!reaction.message.embeds[0] || reaction.message.embeds[0].title !== LANGUAGE_SETUP_TITLE) { return; }
...
async function handleSetupCommand(message) {
    const embed = new EmbedBuilder().setColor('#8b5cf6').setTitle(LANGUAGE_SETUP_TITLE).setDescription('Reacciona a tu bandera para traducir tus mensajes.\n*React with your flag to have your messages translated.*').addFields(Object.values(languageRoles).map(role => ({ name: `${Object.keys(languageRoles).find(key => languageRoles[key] === role)} ${role.name}`, value: ``, inline: true }))).setFooter({ text: 'Solo puedes tener un rol de idioma.' });
    try {
