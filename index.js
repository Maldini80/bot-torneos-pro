// index.js - VERSIÓN 2.4 - LÓGICA DEL USUARIO RESTAURADA CON CORRECCIONES
...
// --- CORRECCIÓN #1: GUARDADO DE ESTADO COMPLETO ---
// La función ahora guarda las 3 variables de estado, solucionando el problema de los IDs en null.
...
// --- CONFIGURACIÓN (Mantenida desde la versión del usuario) ---
...
// --- CORRECCIÓN #2: MANEJADOR DE ERRORES GLOBAL ---
// Esto previene que el bot se caiga por errores como 'Unknown Interaction'.
...
async function crearHiloDePartido(guild, partido, tipoPartido = 'Grupo') {
    ...
    if (!parentChannel || parentChannel.type !== ChannelType.GuildText) {
        console.error(`[ERROR FATAL] El canal padre para hilos (ID: ${MATCH_THREADS_PARENT_ID}) no existe.`);
        return;
    }
    ...
    if (tipoPartido.startsWith('Grupo')) {
        ...
        description = `**Fase Eliminatoria - ${tipoPartido}**`;
    }
    ...
    try {
        const thread = await parentChannel.threads.create({
            ...
            reason: `Partido: ${partido.equipoA.nombre} vs ${partido.equipoB.nombre}`
        });

        partido.threadId = thread.id;

        const memberPromises = [
            thread.members.add(partido.equipoA.capitanId).catch(err => console.error(`No se pudo añadir capitán ${partido.equipoA.capitanTag}.`)),
            thread.members.add(partido.equipoB.capitanId).catch(err => console.error(`No se pudo añadir capitán ${partido.equipoB.capitanTag}.`))
        ];
        ...
        const captainButtons = new ActionRowBuilder().addComponents(
            ...
            new ButtonBuilder().setCustomId(`upload_highlights_${partido.matchId}`).setLabel("Subir Alturas").setStyle(ButtonStyle.Secondary).setEmoji("🎬"),
...
async function mostrarMensajeEspera() {
    ...
    for (const channelId of Object.values(CHANNELS_CONFIG).map(c => c.id)) {
        try {
            const channel = await client.channels.fetch(channelId);
            if (channel) {
...
client.on('guildMemberAdd', member => {
    const welcomeEmbed = new EmbedBuilder().setColor('#2ecc71').setTitle(`👋 ¡Bienvenido a ${member.guild.name}!`).setDescription('🇪🇸 Para continuar, selecciona tu idioma.\n🇬🇧 *To continue, please select your language.*');
...
// --- CORRECCIÓN #2: MANEJADOR DE INTERACCIONES A PRUEBA DE CRASH ---
client.on('interactionCreate', async interaction => {
    try {
        ...
        if (error.code === 10062) {
            console.warn(`[WARN] Interacción expirada (ID: ${interaction.id}) ignorada.`);
            return;
        }
        console.error('Error no controlado en interactionCreate:', error);
        try {
            const replyOptions = { content: '🇪🇸 Hubo un error al procesar tu solicitud.', flags: [MessageFlags.Ephemeral] };
            ...
            if (e.code !== 10062) {
                console.error('Error al enviar el mensaje de error de la interacción:', e);
            }
        }
    }
});
...
async function handleSlashCommand(interaction) {
    ...
    if (commandName === 'panel-admin') {
        const embed = new EmbedBuilder().setColor('#2c3e50').setTitle('Panel de Control del Torneo').setDescription('Usa los botones para gestionar el torneo.').setFooter({ text: 'ESTADO: ✅ LISTO' });
        ...
        const row3 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('panel_simular_partidos').setLabel('Simular Partidos').setStyle(ButtonStyle.Secondary).setEmoji('🎲'), new ButtonBuilder().setCustomId('panel_borrar_hilos').setLabel('Borrar Hilos').setStyle(ButtonStyle.Danger).setEmoji('🗑️'), new ButtonBuilder().setCustomId('panel_finalizar').setLabel('Finalizar Torneo').setStyle(ButtonStyle.Danger).setEmoji('🛑'), new ButtonBuilder().setCustomId('panel_reset_force').setLabel('Reset Forzado').setStyle(ButtonStyle.Danger).setEmoji('🚨'));
        await interaction.channel.send({ embeds: [embed], components: [row1, row2, row3] });
        return interaction.reply({ content: 'Panel creado.', flags: [MessageFlags.Ephemeral] });
    }
    
    if (commandName === 'sortear-grupos') {
        await interaction.deferReply({ flags: [MessageFlags.Ephemeral] });
        if (!torneoActivo) return interaction.editReply({ content: 'No hay torneo activo.' });
        if (torneoActivo.status === 'fase_de_grupos') return interaction.editReply({ content: 'El torneo ya fue sorteado.' });
        const equiposAprobadosCount = Object.keys(torneoActivo.equipos_aprobados || {}).length;
        if (equiposAprobadosCount < torneoActivo.size) return interaction.editReply({ content: `Faltan equipos. Se necesitan ${torneoActivo.size} y hay ${equiposAprobadosCount}.` });
        await interaction.editReply({ content: 'Iniciando sorteo...' });
...
async function forceResetTournamentState(interaction) {
    ...
    try {
        ...
        await interaction.editReply({ content: '❌ Error en el reseteo.' });
    } finally {
        const components = adminPanelMessage.components.map(row => ActionRowBuilder.from(row));
        components.forEach(row => row.components.forEach(button => button.setDisabled(false)));
        const readyEmbed = EmbedBuilder.from(adminPanelMessage.embeds[0]).setColor('#2c3e50').setFooter({ text: 'ESTADO: ✅ LISTO' });
...
async function handleButton(interaction) {
    ...
    if (customId.startsWith('panel_')) {
        ...
        if (type === 'crear') {
             if (torneoActivo) {
                return interaction.reply({ content: '❌ Ya hay un torneo activo.', flags: [MessageFlags.Ephemeral] });
            }
            const formatMenu = new StringSelectMenuBuilder().setCustomId('crear_torneo_format_select').setPlaceholder('Paso 1: Selecciona el formato').addOptions(Object.keys(TOURNAMENT_FORMATS).map(key => ({ label: TOURNAMENT_FORMATS[key].label, description: TOURNAMENT_FORMATS[key].description, value: key })));
        ...
        if (type === 'add' && subtype === 'test') {
            if (!torneoActivo) {
                return interaction.reply({ content: '❌ Error: Primero crea un torneo.', flags: [MessageFlags.Ephemeral] });
            }
            const modal = new ModalBuilder().setCustomId('add_test_modal').setTitle('Añadir Equipos de Prueba');
            const cantidadInput = new TextInputBuilder().setCustomId('cantidad_input').setLabel("¿Cuántos equipos quieres añadir?").setStyle(TextInputStyle.Short).setRequired(true);
        ...
        if (type === 'finalizar') {
            if (!torneoActivo) return interaction.editReply({ content: 'No hay torneo activo para finalizar.' });
            ...
            try {
                ...
                await interaction.editReply({ content: 'Finalizando... Limpiando hilos...' });
                const parentChannel = await client.channels.fetch(MATCH_THREADS_PARENT_ID).catch(() => null);
                if (parentChannel) {
                ...
                await interaction.followUp({ content: '✅ Torneo finalizado y reseteado.', flags: [MessageFlags.Ephemeral] });
            } catch (error) {
                console.error("Error al finalizar torneo:", error);
                await interaction.followUp({ content: '❌ Error al finalizar.', flags: [MessageFlags.Ephemeral] });
            } finally {
                const components = adminPanelMessage.components.map(row => ActionRowBuilder.from(row));
                components.forEach(row => row.components.forEach(button => button.setDisabled(false)));
                const readyEmbed = EmbedBuilder.from(adminPanelMessage.embeds[0]).setColor('#2c3e50').setFooter({ text: `ESTADO: ✅ LISTO` });
        ...
        if (type === 'ver' && subtype === 'inscritos') {
            if (!torneoActivo || Object.keys(torneoActivo.equipos_aprobados || {}).length === 0) return interaction.editReply({ content: 'No hay equipos inscritos.' });
            const listaEquipos = Object.values(torneoActivo.equipos_aprobados).map((equipo, index) => `${index + 1}. ${equipo.bandera || '🏳️'} **${equipo.nombre}** (Cap: ${equipo.capitanTag})`).join('\n');
            const embed = new EmbedBuilder().setTitle('📋 Equipos Inscritos').setDescription(listaEquipos).setColor('#3498DB').setFooter({ text: `Total: ${Object.keys(torneoActivo.equipos_aprobados).length} / ${torneoActivo.size}` });
            return interaction.editReply({ embeds: [embed] });
        }

        if (type === 'ver' && subtype === 'pendientes') {
            if (!torneoActivo || Object.keys(torneoActivo.equipos_pendientes || {}).length === 0) return interaction.editReply({ content: 'No hay equipos pendientes.' });
            const listaPendientes = Object.values(torneoActivo.equipos_pendientes).map((equipo, index) => `${index + 1}. **${equipo.nombre}** (Cap: ${equipo.capitanTag}) - PayPal: \`${equipo.paypal || 'N/A'}\``).join('\n');
            const embed = new EmbedBuilder().setTitle('⏳ Equipos Pendientes').setDescription(listaPendientes).setColor('#E67E22');
            return interaction.editReply({ embeds: [embed] });
        }
        
        if (type === 'simular' && subtype === 'partidos') {
            if (!torneoActivo || !['fase_de_grupos', 'octavos', 'cuartos', 'semifinales', 'final'].includes(torneoActivo.status)) return interaction.editReply({ content: 'Solo se puede simular durante una fase activa.' });
            let partidosASimular = findMatch(null, true).filter(p => p.threadId && p.status !== 'finalizado');
            if (partidosASimular.length === 0) return interaction.editReply({ content: 'No hay partidos activos para simular.' });
            
            await interaction.editReply({ content: `Simulando ${partidosASimular.length} partidos...` });
            ...
            saveBotState();
            await interaction.followUp({ content: `✅ ${partidosASimular.length} partidos simulados.` });
            return;
        } 
        ...
        if (type === 'borrar' && subtype === 'hilos') {
            ...
            try {
                ...
                const threads = await parentChannel.threads.fetch();
                await interaction.editReply({ content: `Borrando ${threads.threads.size} hilos...` });
                const deletePromises = [];
                for (const thread of threads.threads.values()) {
                ...
                if (torneoActivo) {
                    ...
                    Object.keys(torneoActivo.eliminatorias).forEach(fase => {
                        if(Array.isArray(torneoActivo.eliminatorias[fase])) torneoActivo.eliminatorias[fase].forEach(p => p.threadId = null);
                        else if (torneoActivo.eliminatorias[fase]?.threadId) torneoActivo.eliminatorias[fase].threadId = null;
                    });
                    saveBotState();
                }
                await interaction.followUp({ content: `✅ ${results.length} hilos borrados.`, flags: [MessageFlags.Ephemeral] });

            } catch(error) {
                 console.error("Error al borrar hilos:", error);
                 await interaction.followUp({ content: `❌ Error al borrar hilos.`, flags: [MessageFlags.Ephemeral] });
            } finally {
                const components = adminPanelMessage.components.map(row => ActionRowBuilder.from(row));
                components.forEach(row => row.components.forEach(button => button.setDisabled(false)));
                const readyEmbed = EmbedBuilder.from(adminPanelMessage.embeds[0]).setColor('#2c3e50').setFooter({ text: `ESTADO: ✅ LISTO` });
        ...
        if(!isDeferred) await interaction.deferReply({ flags: [MessageFlags.Ephemeral] });
        const [, langCode, guildId] = customId.split('_');
        const roleInfo = Object.values(languageRoles).find(r => r.code === langCode);
        const guild = await client.guilds.fetch(guildId).catch(() => null);
        const member = await guild?.members.fetch(interaction.user.id).catch(() => null);
        if (!roleInfo || !guild || !member) return interaction.editReply({ content: 'Error al procesar el botón.' });
        ...
        if (!torneoActivo || torneoActivo.status !== 'inscripcion_abierta') return interaction.reply({ content: '🇪🇸 Las inscripciones no están abiertas.', flags: [MessageFlags.Ephemeral] });
        ...
        if(!partido) return interaction.reply({content: "Error: No se encontró el partido.", flags: [MessageFlags.Ephemeral] });
        if (partido.status === 'finalizado') return interaction.reply({content: "Este partido ya tiene resultado.", flags: [MessageFlags.Ephemeral] });
        ...
        const modal = new ModalBuilder().setCustomId(`highlights_modal_${customId.replace('upload_highlights_', '')}`).setTitle('Subir Alturas');
        ...
        if (customId.startsWith('solicitar_arbitraje_')) {
            ...
            const { partido } = findMatch(matchId);
            if(!partido) return interaction.editReply({content: "Error: No se encontró el partido." });
            if(partido.status !== 'finalizado') {
                ...
                return interaction.editReply({ content: `No se puede solicitar arbitraje para un partido finalizado.`});
            }
        }
    
        if (customId.startsWith('admin_aprobar_') || customId.startsWith('admin_rechazar_') || customId.startsWith('admin_expulsar_')) {
            if (!torneoActivo) return interaction.editReply({ content: "Error: El torneo ya no existe." });
            const [, type, captainId] = customId.split('_');
            
            if (type === 'expulsar') {
                if (torneoActivo.status !== 'inscripcion_abierta') return interaction.editReply({ content: 'Solo se puede expulsar durante la inscripción.' });
                const teamToKick = torneoActivo.equipos_aprobados[captainId];
                if (!teamToKick) return interaction.editReply({ content: 'Error: Equipo no encontrado.' });
                ...
                if(captainUser) await captainUser.send(`🇪🇸 Tu equipo **${teamToKick.nombre}** ha sido eliminado del torneo.`).catch(() => {});
                ...
                if (type === 'aprobar') {
                    if (Object.keys(torneoActivo.equipos_aprobados).length >= torneoActivo.size) return interaction.editReply({ content: 'El cupo está lleno.' });
                    ...
                    if (captainMember) {
                        const participanteRole = await interaction.guild.roles.fetch(PARTICIPANTE_ROLE_ID).catch(()=>null);
                    ...
                    const captainUser = await client.users.fetch(captainId).catch(()=>null);
                    if(captainUser) await captainUser.send(`✅ ¡Tu inscripción para **${equipoPendiente.nombre}** ha sido aprobada!`).catch(()=>{});
                    ...
                    if (Object.keys(torneoActivo.equipos_aprobados).length === torneoActivo.size) {
                        await interaction.followUp({ content: `¡Cupo de ${torneoActivo.size} equipos lleno! Sorteando...`, flags: [MessageFlags.Ephemeral] });
                        await realizarSorteoDeGrupos(interaction.guild);
                    }
                } else { // Rechazar
                    delete torneoActivo.equipos_pendientes[captainId];
        ...
        if (customId.startsWith('admin_confirm_payment_')) {
            ...
            if (!winner) return interaction.editReply({ content: 'No se encontró al ganador.' });
            
            const dmEmbed = new EmbedBuilder().setColor('#2ECC71').setTitle('💸 ¡Premio Recibido!').setDescription(`¡Felicidades! El premio del torneo **${torneoActivo.nombre}** ha sido enviado.`);
            try {
                await winner.send({ embeds: [dmEmbed] });
            } catch (e) {
                return interaction.editReply({ content: `No se pudo enviar el DM al ganador.` });
            }
            const originalMessage = interaction.message;
            ...
            disabledRow.components.forEach(component => {
                if (component.data.custom_id === customId) component.setDisabled(true).setLabel('Pago Confirmado');
            });
            await originalMessage.edit({ embeds: [newEmbed], components: [disabledRow] });
            await interaction.editReply({ content: `✅ Notificación de pago enviada.` });
        }
    }
}
...
async function handleSelectMenu(interaction) {
    if (interaction.customId === 'crear_torneo_format_select') {
        ...
        const typeMenu = new StringSelectMenuBuilder().setCustomId(`crear_torneo_type_select_${formatId}`).setPlaceholder('Paso 2: Selecciona el tipo').addOptions([{ label: 'De Pago', value: 'pago' }, { label: 'Gratuito', value: 'gratis' }]);
        const row = new ActionRowBuilder().addComponents(typeMenu);
        await interaction.update({ content: `Formato: **${format.label}**. Ahora, el tipo:`, components: [row] });
        ...
        const modal = new ModalBuilder().setCustomId(`crear_torneo_final_${formatId}_${type}`).setTitle('Finalizar Creación');
        const nombreInput = new TextInputBuilder().setCustomId('torneo_nombre').setLabel("Nombre del Torneo").setStyle(TextInputStyle.Short).setRequired(true);
        modal.addComponents(new ActionRowBuilder().addComponents(nombreInput));
        if (type === 'pago') {
            const paypalInput = new TextInputBuilder().setCustomId('torneo_paypal').setLabel("Enlace PayPal.Me").setStyle(TextInputStyle.Short).setRequired(true);
...
async function handleModalSubmit(interaction) {
    ...
    if (customId.startsWith('crear_torneo_final_')) {
        try {
            if (torneoActivo) return interaction.editReply({ content: '❌ Ya hay un torneo activo.' });
            
            const match = customId.match(/crear_torneo_final_(.+)_(pago|gratis)/);
            if (!match) return interaction.editReply({ content: 'Error: `customId` inválido.' });
            const [, formatId, type] = match;
            
            const format = TOURNAMENT_FORMATS[formatId];
            if (!format) return interaction.editReply({ content: 'Error: Formato inválido.' });
            ...
            if (isPaid) {
                ...
                if (!enlacePaypal || isNaN(prizeCampeon) || isNaN(prizeFinalista)) return interaction.editReply({ content: 'Datos de pago inválidos.' });
            }
            
            await interaction.editReply({ content: `✅ Recibido. Creando torneo "${nombre}"...` });

            await Promise.all(Object.values(CHANNELS_CONFIG).map(c => limpiarCanal(c.id)));
            ...
            torneoActivo = { 
                ...
                publicGroupsMessageId: null, calendarioMessageId: null,
            ...
            const embedLista = new EmbedBuilder().setColor('#3498db').setTitle(`Equipos Inscritos - ${nombre}`).setDescription('Aún no hay equipos.').setFooter({ text: `Total: 0 / ${torneoActivo.size}` });
            ...
            if (calendarioChannel) {
                const embedCalendario = new EmbedBuilder().setColor('#9b59b6').setTitle(`🗓️ Calendario - ${nombre}`).setDescription('El calendario aparecerá aquí tras el sorteo.');
            ...
            await interaction.followUp({ content: `✅ Torneo "${nombre}" creado con éxito.`, ephemeral: true });

        } catch (error) {
            console.error("Error crítico al crear torneo:", error);
            torneoActivo = null; 
            saveBotState();
            await interaction.followUp({ content: '❌ Error inesperado al crear el torneo.', ephemeral: true });
        }
    } 
        ...
        const allTeamNames = [...Object.values(torneoActivo.equipos_aprobados || {}).map(e => e.nombre.toLowerCase()), ...Object.values(torneoActivo.equipos_pendientes || {}).map(e => e.nombre.toLowerCase())];
        if (allTeamNames.includes(teamName.toLowerCase())) return interaction.editReply({ content: 'Ese nombre de equipo ya existe.' });
        ...
        if (torneoActivo.isPaid) {
            const embed = new EmbedBuilder().setColor('#f1c40f').setTitle('Inscripción Recibida - Pendiente de Pago').addFields({ name: 'Enlace de Pago', value: torneoActivo.enlace_paypal }, { name: 'Siguiente Paso', value: "Cuando pagues, haz clic abajo." });
            ...
            try {
                ...
                await interaction.editReply({ content: '❌ No pude enviarte un DM. Revisa tu privacidad.' });
            }
        } else {
        ...
        if (!torneoActivo) return interaction.editReply({ content: '❌ El torneo ya no existe.' });
        ...
        for (const flag in languageRoles) {
            ...
            if (role && interaction.member.roles.cache.has(role.id)) { adminFlag = flag; break; }
        }
        ...
        for (let i = 0; i < cantidad; i++) {
            ...
            torneoActivo.equipos_aprobados[teamId] = { id: teamId, nombre: `E-Prueba-${initialCount + i + 1}`, capitanId: interaction.user.id, capitanTag: interaction.user.tag, bandera: adminFlag, paypal: 'admin@test.com' };
        }
        saveBotState();
        ...
        await interaction.followUp({ content: '✅ Interfaz actualizada.', ephemeral: true });
        ...
        if (isNaN(golesA) || isNaN(golesB)) return interaction.editReply("Formato de resultado inválido.");
        
        const { partido } = findMatch(matchId);
        if (!partido) return interaction.editReply("Error: Partido no encontrado.");
        if (partido.resultado) return interaction.editReply("Este partido ya tiene resultado.");
        ...
        if (otherCaptainResult) {
            if (otherCaptainResult.golesA === golesA && otherCaptainResult.golesB === golesB) {
                partido.resultado = `${golesA}-${golesB}`;
                partido.status = 'finalizado';
                await interaction.editReply(`✅ Resultado ${partido.resultado} confirmado por ambos.`);
                await procesarResultadoFinal(partido, interaction);
            } else {
        ...
        if (isNaN(golesA) || isNaN(golesB)) return interaction.editReply("Formato inválido.");
        ...
        const embed = new EmbedBuilder().setColor('#7289DA').setAuthor({ name: `Alturas de ${interaction.user.tag}`, iconURL: interaction.user.displayAvatarURL() }).setTitle('🎬 ¡Nuevas Alturas!').setDescription(`**[Ver Clip](${link})**\n\n${description || 'Sin descripción'}`).setTimestamp();
        
        try {
            await interaction.channel.send({ embeds: [embed] });
            await interaction.editReply({ content: '✅ Alturas compartidas.' });
        } catch (e) {
...
async function procesarResultadoFinal(partido, interaction, fromSimulation = false) {
    ...
    if (!fromSimulation) {
        ...
        if (thread) await thread.send({ content: `✅ Resultado final: **${partido.equipoA.nombre} ${partido.resultado} ${partido.equipoB.nombre}**.`});
    }
}
...
async function realizarSorteoDeGrupos(guild) {
    ...
    if (!format) return console.error("Error: Formato de torneo no encontrado.");

    const adminChannel = await client.channels.fetch(ADMIN_CHANNEL_ID).catch(() => null);
    if (adminChannel) await adminChannel.send('Iniciando sorteo...');
    ...
    if(mensajeInscripcionId) {
        try {
            ...
            await msg.edit({ content: 'Inscripciones cerradas.', components: [disabledRow] });
    ...
    for (const nombreGrupo in grupos) {
        ...
        if (equiposGrupo.length === 4) {
            ...
            calendario[nombreGrupo].push(createMatchObject(nombreGrupo, 1, team1, team2), createMatchObject(nombreGrupo, 1, team3, team4), createMatchObject(nombreGrupo, 2, team1, team3), createMatchObject(nombreGrupo, 2, team2, team4), createMatchObject(nombreGrupo, 3, team1, team4), createMatchObject(nombreGrupo, 3, team2, team3));
        }
    }
    ...
    if (!gruposChannel) return console.error(`Error: Canal de clasificación no encontrado.`);
    
    await limpiarCanal(CLASIFICACION_CHANNEL_ID);
    const embedClasificacion = new EmbedBuilder().setColor('#1abc9c').setTitle(`Clasificación: ${torneo.nombre}`).setTimestamp();
    ...
    saveBotState();
    if(adminChannel) await adminChannel.send(`✅ Sorteo completado.`);
}
...
async function verificarYCrearSiguientesHilos(guild) {
    ...
    for (const groupName in torneoActivo.calendario) {
        ...
        if (partidosJornadaAnterior.length > 0 && partidosJornadaAnterior.every(p => p.status === 'finalizado')) {
             for(const partido of torneoActivo.calendario[groupName].filter(p => p.jornada === primeraJornadaPendiente)) {
                if(!partido.threadId) await crearHiloDePartido(guild, partido, `Grupo ${groupName.slice(-1)}`);
             }
        }
    }
    saveBotState();
...
function crearPartidosEliminatoria(equipos, ronda) {
    ...
    for(let i = 0; i < equipos.length; i += 2) {
        ...
        partidos.push({ matchId: `match_${ronda}_${i/2}_${Date.now()}`, equipoA: equipos[i], equipoB: equipos[i+1], resultado: null, reportedScores: {}, status: 'en_curso', nombreGrupo: null, ronda: ronda });
    }
    return partidos;
}

async function iniciarFaseEliminatoria(guild) {
    if (!torneoActivo || torneoActivo.status !== 'fase_de_grupos' || !Object.values(torneoActivo.calendario).flat().every(p => p.status === 'finalizado')) return;
    ...
    const clasificados = [];
    for (const groupName of Object.keys(torneoActivo.grupos).sort()) {
        const grupoOrdenado = [...torneoActivo.grupos[groupName].equipos].sort((a,b) => sortTeams(a,b,groupName));
        const clasificadosDelGrupo = grupoOrdenado.slice(0, format.qualifiersPerGroup).map(c => JSON.parse(JSON.stringify(c)));
        clasificados.push(...clasificadosDelGrupo);
    }
    
    const partidos = crearPartidosEliminatoria(clasificados, primeraRonda);
    if (primeraRonda === 'final') torneoActivo.eliminatorias.final = partidos[0];
    else torneoActivo.eliminatorias[primeraRonda] = partidos;
    
    const clasifChannel = await client.channels.fetch(torneoActivo.canalGruposId).catch(() => null);
    const embedAnuncio = new EmbedBuilder().setColor('#e67e22').setTitle(`🔥 ¡Comienza la Fase Eliminatoria: ${primeraRonda.charAt(0).toUpperCase() + primeraRonda.slice(1)}! 🔥`);
    for(let i = 0; i < partidos.length; i++) {
        await crearHiloDePartido(guild, partidos[i], `${primeraRonda.charAt(0).toUpperCase() + primeraRonda.slice(1)}-${i+1}`);
        embedAnuncio.addFields({ name: `Partido ${i+1}`, value: `> ${partidos[i].equipoA.nombre} vs ${partidos[i].equipoB.nombre}` });
    }
...
async function procesarResultadoEliminatoria(guild) {
    ...
    if (rondaActual === 'final' && torneoActivo.eliminatorias.final?.status === 'finalizado') return await handleFinalResult();

    const partidosRonda = torneoActivo.eliminatorias[rondaActual];
    if (!partidosRonda || !partidosRonda.every(p => p.status === 'finalizado')) return;
    ...
    if (!siguienteRonda) return console.error("Error: No se encontró la siguiente ronda.");
    ...
    const ganadores = partidosRonda.map(p => {
        ...
        return JSON.parse(JSON.stringify(golesA > golesB ? p.equipoA : p.equipoB));
    });

    const partidos = crearPartidosEliminatoria(ganadores, siguienteRonda);
    if (siguienteRonda === 'final') torneoActivo.eliminatorias.final = partidos[0];
    else torneoActivo.eliminatorias[siguienteRonda] = partidos;
    
    const clasifChannel = await client.channels.fetch(torneoActivo.canalGruposId).catch(() => null);
    const embedAnuncio = new EmbedBuilder().setColor('#f1c40f').setTitle(`🏆 ¡Avanzamos a: ${siguienteRonda.charAt(0).toUpperCase() + siguienteRonda.slice(1)}! 🏆`);
    for(let i = 0; i < partidos.length; i++) {
        await crearHiloDePartido(guild, partidos[i], `${siguienteRonda.charAt(0).toUpperCase() + siguienteRonda.slice(1)}-${i+1}`);
        embedAnuncio.addFields({ name: `Partido ${i+1}`, value: `> ${partidos[i].equipoA.nombre} vs ${partidos[i].equipoB.nombre}` });
    }
    if(clasifChannel) await clasifChannel.send({ embeds: [embedAnuncio] });
    saveBotState();
...
async function handleFinalResult() {
    ...
    const embedCampeon = new EmbedBuilder().setColor('#ffd700').setTitle(`🎉 ¡Tenemos Campeón!`).setDescription(`**¡Felicidades a ${campeon.nombre} por ganar el torneo!**`).setThumbnail('https://i.imgur.com/C5mJg1s.png').setTimestamp();
    const clasifChannel = await client.channels.fetch(torneoActivo.canalGruposId).catch(() => null);
    ...
    if (torneoActivo.isPaid) {
        ...
        if(adminChannel) {
            ...
            if (campeon.paypal) rowCampeon.addComponents(new ButtonBuilder().setLabel('Pagar').setStyle(ButtonStyle.Link).setURL(`https://paypal.me/${encodeURIComponent(campeon.paypal)}/${torneoActivo.prizeCampeon}`));
            ...
            const rowFinalista = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`admin_confirm_payment_finalista_${finalista.id}`).setLabel('Confirmar Pago Finalista').setStyle(ButtonStyle.Success));
             if (finalista.paypal) rowFinalista.addComponents(new ButtonBuilder().setLabel('Pagar').setStyle(ButtonStyle.Link).setURL(`https://paypal.me/${encodeURIComponent(finalista.paypal)}/${torneoActivo.prizeFinalista}`));
...
async function actualizarMensajeClasificacion() {
    ...
    for (const groupName in torneoActivo.grupos) {
        const equiposOrdenados = [...torneoActivo.grupos[groupName].equipos].sort((a,b) => sortTeams(a,b,groupName));
        const nameWidth = 16;
...
async function actualizarMensajeCalendario() {
    ...
    if (!message) return;
    const newEmbed = EmbedBuilder.from(message.embeds[0]).setDescription('Calendario del torneo.').setFields([]);
    let allFields = [];
    for (const groupName of Object.keys(torneoActivo.calendario).sort()) {
        ...
        for (const jornadaNum of Object.keys(partidosPorJornada).sort((a,b) => a - b)) {
            ...
            for (const partido of partidosPorJornada[jornadaNum]) {
                ...
                const paddedCenter = ' '.repeat(Math.ceil(paddingTotal / 2)) + centerText + ' '.repeat(Math.floor(paddingTotal / 2));
...
client.on('messageCreate', async message => {
    ...
    if (message.content === SETUP_COMMAND && message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
...
client.on('messageReactionAdd', async (reaction, user) => {
    ...
    if (!reaction.message.embeds[0]?.title?.includes('Selección de Idioma')) return;
    const roleInfo = languageRoles[reaction.emoji.name];
    if (!roleInfo) return;
    ...
    try {
        const rolesToRemove = member.roles.cache.filter(role => Object.values(languageRoles).some(lang => lang.name === role.name));
        if (rolesToRemove.size > 0) await member.roles.remove(rolesToRemove);
        const roleToAdd = reaction.message.guild.roles.cache.find(r => r.name === roleInfo.name);
        if (roleToAdd) await member.roles.add(roleToAdd);
    } catch (error) { console.error('Error al asignar rol por reacción:', error); }
});

async function handleSetupCommand(message) {
    const embed = new EmbedBuilder().setColor('#8b5cf6').setTitle(LANGUAGE_SETUP_TITLE).setDescription('Reacciona para traducir tus mensajes.').addFields(Object.values(languageRoles).map(role => ({ name: `${Object.keys(languageRoles).find(key => languageRoles[key] === role)} ${role.name}`, value: `\u200b`, inline: true }))).setFooter({ text: 'Solo un rol de idioma por persona.' });
    try {
        const sentMessage = await message.channel.send({ embeds: [embed] });
        for (const flag in languageRoles) await sentMessage.react(flag);
        await message.delete();
    } catch (error) { console.error('Error en setup:', error); }
}

async function startBot() {
    console.log('[INIT] Conectando a DB...');
    await connectDb();
    console.log('[INIT] Cargando datos...');
...
startBot();--- START OF FILE index.js v1.txt ---
