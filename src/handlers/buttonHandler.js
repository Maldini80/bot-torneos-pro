...
export async function handleButton(interaction) {
    ...
    if (modalActions.includes(action)) {
        ...
        if (action === 'inscribir_equipo_start') {
            ...
            modal.addComponents(
                new ActionRowBuilder().addComponents(prizeCInput), new ActionRowBuilder().addComponents(prizeFInput), new ActionRowBuilder().addComponents(feeInput)
            );
    ...
    // --- GRUPO 2: Acciones de coordinación ---
    if (action === 'im_ready') {
        ...
        const localTeam = partido.equipoA;
        const awayTeam = partido.equipoB;

        const invitationButton = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`invitation_sent:${matchId}:${tournamentShortId}`).setLabel("He Enviado la Invitación / Invitation Sent").setStyle(ButtonStyle.Success)
        );

        await interaction.channel.send({
            content: `👋 <@${interaction.user.id}> está listo para jugar. <t:${Math.floor(Date.now() / 1000)}:T>\n` +
                     `El equipo local es **${localTeam.nombre}**. Su nombre en EAFC es \`${localTeam.eafcTeamName}\`.\n` +
                     `<@${awayTeam.capitanId}>, por favor, envía la invitación y pulsa el botón de abajo.`,
            components: [invitationButton]
        });
        
        const originalMessage = interaction.message;
        const newComponents = originalMessage.components.map(row => {
            const newRow = ActionRowBuilder.from(row);
            newRow.components.forEach(comp => { if (comp.customId === customId) comp.setDisabled(true); });
            return newRow;
        });
    ...
    if (action === 'admin_change_format_start') {
        ...
        const formatMenu = new StringSelectMenuBuilder()
            .setCustomId(`admin_change_format_select:${tournamentShortId}`)
            .setPlaceholder('Selecciona el nuevo formato')
            .addOptions(Object.keys(TOURNAMENT_FORMATS).map(key => ({ label: TOURNAMENT_FORMATS[key].label, value: key })));
        const typeMenu = new StringSelectMenuBuilder()
            .setCustomId(`admin_change_type_select:${tournamentShortId}`)
            .setPlaceholder('Selecciona el nuevo tipo de pago')
            .addOptions([ { label: 'Gratuito', value: 'gratis' }, { label: 'De Pago', value: 'pago' } ]);

        await interaction.editReply({ 
            content: `**Editando:** ${tournament.nombre}\nSelecciona el nuevo formato o tipo.`,
