...
export async function handleModal(interaction) {
    ...
    if (action === 'add_whatsapp_to_profile_modal') {
        ...
        // 1. Crear registro verificado (Auto-verificación)
        ...
        if (!verifiedUser) {
            ...
            try {
                ...
                console.warn(`[Modal Auto-Verification] Error assigning role: ${roleErr.message}`);
            }
        }

        // 2. Registrar en la lista de jugadores del draft
        const playerData = {
            userId,
            userName: interaction.user.username,
            psnId,
...
Último Puesto: **${newLeague.last > 0 ? '+'+newLeague.last : newLeague.last}**` }
    ...
    if (action === 'add_test_teams_modal') {
        ...
        for (let i = 0; i < amountToAdd; i++) {
            ...
            bulkOps.push({ updateOne: { filter: { _id: tournament._id }, update: { $set: { [`teams.aprobados.${capitanId}`]: teamData } } } });
        }
    ...
    if (customId.startsWith('paid_link_ea_modal_')) {
        ...
        try {
            ...
            const options = clubs.slice(0, 25).map(c => {
                ...
                const safeName = name.substring(0, 50).replace(/\|/g, ''); // max 50 chars
                return {
