...
export async function generateRandomSquadForTeam(db, leagueId, teamId) {
    ...
    if (poolPOR.length < 1 || poolCB.length < 3 || poolDC.length < 3 || poolMCStrict.length < 4 || poolCARR.length < 3) {
        throw new Error(`No hay suficientes jugadores en la base de datos para la(s) división(es) de esta liga (${vpgLeagues.join(', ')}). Por favor, asegúrate de sincronizar la división en el bot antes de continuar.`);
    }
