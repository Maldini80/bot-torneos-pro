...
const ruleEmbeds = [
    new EmbedBuilder()
        ...
        .setDescription(
            "**• Salidas del Partido:**\n" +
            "Se permite un máximo de 2 salidas por equipo, antes del minuto 10 del partido. Salir del partido una tercera vez es sancionable.\n\n" +
            "**• Tiempo de Cortesía:**\n" +
            "Cortesía máxima de 10 minutos.\n\n" +
            "**• Límites de Altura:**\n" +
            "DFC (Defensa): Máximo 187cm.\n" +
            "Resto de Jugadores: Máximo 177cm.\n" +
            "GK (Portero): Sin límite de altura.\n\n" +
            "**• PlayStyles Prohibidos (Plata y Oro):**\n" +
            "AÉREO (Aerial)\n" +
            "ANTICIPACIÓN (Anticipate / \"Mapache\")"
        ),
    new EmbedBuilder()
        .setColor('#f1c40f')
        .setTitle('📋 REPORTES Y SANCIONES')
        .setDescription(
            "**GUÍA DE REPORTES Y PRUEBAS**\n\n" +
            "**• Paso 1:** Al finalizar, pulsa el botón 'Reportar Resultado' en el hilo del partido.\n\n" +
            "**• Paso 2:** Debes tener un clip que muestre el vestíbulo final (jugadores, alturas, perks). **Solo se presenta si el rival lo solicita.**\n\n" +
            "**• Paso 3:** Si detectas una irregularidad en el vídeo del rival, pulsa 'Solicitar Arbitraje'.\n\n" +
            "-------------------------------------\n\n" +
            "**⚠️ SANCIONES POR INCUMPLIMIENTO**\n\n" +
            "**• No cumplir el tiempo de cortesía:**\n" +
            "  Consecuencia: Partido perdido 1-0.\n\n" +
            "**• Pruebas de Infracción (Altura/Perk):**\n" +
            "  Consecuencia: Partido perdido 3-0.\n\n" +
            "**• No Presentar Pruebas (si se solicitan):**\n" +
            "  Consecuencia: Partido perdido 3-0."
        )
];
...
export function createTournamentManagementPanel(tournament, isBusy = false) {
    ...
    if (isBeforeDraw) {
        row1.addComponents(
            new ButtonBuilder().setCustomId(`admin_edit_tournament_start:${tournament.shortId}`).setLabel('Editar Torneo').setStyle(ButtonStyle.Primary).setEmoji('📝').setDisabled(isBusy),
