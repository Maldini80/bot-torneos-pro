...
🚫Sin PlayStyles Prohibidos), new EmbedBuilder() .setColor('#f1c40f') .setTitle('📋 GUÍA DE REPORTES, PRUEBAS Y DISPUTAS') .setDescription(• Al finalizar el partido, ambos capitanes debéis pulsar el botón 'Reportar Resultado' y poner el resultado.

• Si detectas una irregularidad, pulsar el botón 'Solicitar Arbitraje'
y explicar el problema a los árbitros en el hilo.), new EmbedBuilder() .setColor('#f1c40f') .setTitle('⚠️ SANCIONES POR INCUMPLIMIENTO') .setDescription(Las siguientes acciones conllevarán sanciones directas:
...
embed.addFields(
    ...
    { name: 'Equipos', value: `${teamsCount} / ${format.size === 0 ? '∞' : format.size}`, inline: true }
);
...
{ name: 'ID Capitán', value: \
{teamData.twitter.startsWith('http') ? '' : 'https://twitter.com/'}${teamData.twitter})` : 'No proporcionado', inline: true },
