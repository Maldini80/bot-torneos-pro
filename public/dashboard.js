...
class DashboardApp {
    ...
    async openTeamManagement(teamId, teamName, logoUrl) {
        ...
        // === CO-CAPTAIN INVITE (usando mismo patrón que invite-player) ===
        const coCaptainInput = document.getElementById('tm-cocaptain-input');
        const sendCoCaptainBtn = document.getElementById('tm-send-cocaptain-invite');
        const statusMsg = document.getElementById('cocaptain-invite-status');

        if (coCaptainInput) {
            coCaptainInput.value = '';
            statusMsg.textContent = '';

            // Autocomplete Logic (igual que invite-player)
            let debounceTimerCC;
            coCaptainInput.oninput = (e) => {
                const query = e.target.value;
                if (query.length < 2) return;
                ...
                debounceTimerCC = setTimeout(async () => {
                    try {
                        ...
                        console.error('Error searching co-captain:', err);
                    }
                }, 300);
            };

            // Enviar Invitación
            sendCoCaptainBtn.onclick = async () => {
                const coCaptainId = coCaptainInput.value;
                if (!coCaptainId) {
                    statusMsg.textContent = 'Por favor selecciona un usuario';
                    statusMsg.className = 'status-message error';
                    return;
                }

                sendCoCaptainBtn.disabled = true;
                const originalText = sendCoCaptainBtn.textContent;
                sendCoCaptainBtn.textContent = '...';
                statusMsg.textContent = '';

                try {
                    const res = await fetch(`/api/teams/${teamId}/invite-co-captain`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ coCaptainId, lang: this.currentLang })
                    });
                    const data = await res.json();
                    ...
                    sendCoCaptainBtn.textContent = originalText;
                    sendCoCaptainBtn.disabled = false;
                }
            };
        }
