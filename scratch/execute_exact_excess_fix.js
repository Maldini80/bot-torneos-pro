...
async function main() {
    ...
    console.log(EXECUTE ? '=== MODO EJECUCIÓN (APLICANDO CAMBIOS) ===\n' : '=== MODO SIMULACIÓN (DRY RUN) ===\n');
    ...
    for (const team of teams) {
        ...
        if (teamExcessPoints > 0) {
            ...
            totalBalanceSubtracted += excessBalance;
            ...
            console.log(`  Presupuesto: ${currentBalance.toLocaleString()} € -> ${correctedBalance.toLocaleString()} € (-${excessBalance.toLocaleString()} €)`);

            if (EXECUTE) {
                // 1. Actualizar puntos y balance del equipo
                ...
                // 2. Registrar noticia explicativa en fantasy_news
                const newsMsg = `🔧 **CORRECCIÓN DUPLICIDAD**: El equipo **${team.teamName}** ha sido corregido restando **${teamExcessPoints}** pts y **${excessBalance.toLocaleString('es-ES')} €** del presupuesto. Motivo: corrección del bug de duplicación de estadísticas de jugadores multi-liga (${playersContributed.map(p => p.name).join(', ')}).`;
                ...
                await db.collection('fantasy_news').insertOne({
                    ...
                    metadata: {
                        ...
                        balanceCorrected: -excessBalance,
                        players: playersContributed.map(p => p.name),
                        timestamp: new Date().toISOString()
                    },
                    createdAt: new Date()
                });
                console.log(`  [DB] Cambios aplicados y noticia registrada.`);
            }
            console.log('----------------------------------------------------');
        }
    }
    ...
    console.log(`Total balance a restar: ${totalBalanceSubtracted.toLocaleString('es-ES')} €`);
